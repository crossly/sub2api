...
func (s *OpenAIGatewayService) isOpenAIAccountEligibleForRequest(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) bool {
	...
	if s != nil && s.getOpenAIOverLimitModeSettings(ctx).Enabled {
		if !s.isOpenAIAccountSelectableForRequest(ctx, account, requestedModel, requireCompact) {
			return false
		}
...
func (s *OpenAIGatewayService) selectAccountForModelWithExclusions(ctx context.Context, groupID *int64, sessionHash string, requestedModel string, excludedIDs map[int64]struct{}, requireCompact bool, stickyAccountID int64, requiredCapability OpenAIEndpointCapability) (*Account, error) {
	...
	hydrated, err := s.hydrateSelectedAccount(ctx, selected, requiredCapability)
	if err != nil {
		return nil, err
	}
...
func (s *OpenAIGatewayService) resolveFreshSchedulableOpenAIAccount(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
	...
	if s.schedulerSnapshot != nil {
		...
		if err == nil && current != nil {
			...
			latest := s.resolveOpenAIOverLimitCandidateFromDB(ctx, account, requestedModel, requireCompact, requiredCapability)
			if latest == nil {
				return nil
			}
			fresh = latest
...
func (s *OpenAIGatewayService) resolveOpenAIOverLimitCandidateFromDB(ctx context.Context, account *Account, requestedModel string, requireCompact bool, requiredCapability OpenAIEndpointCapability) *Account {
...
func (s *OpenAIGatewayService) hydrateSelectedAccount(ctx context.Context, account *Account, requiredCapability OpenAIEndpointCapability) (*Account, error) {
	if account == nil || s.schedulerSnapshot == nil {
		return account, nil
	}
	hydrated, err := s.schedulerSnapshot.GetAccount(ctx, account.ID)
	if err != nil {
		if latest := s.resolveOpenAIOverLimitCandidateFromDB(ctx, account, "", false, requiredCapability); latest != nil {
			return latest, nil
		}
		return nil, err
	}
...
func (s *OpenAIGatewayService) Forward(ctx context.Context, c *gin.Context, account *Account, body []byte) (*OpenAIForwardResult, error) {
	...
	if account.IsOpenAIAndroidMobile() {
		return s.forwardOpenAIAndroidMobile(ctx, c, account, body, startTime)
	}
