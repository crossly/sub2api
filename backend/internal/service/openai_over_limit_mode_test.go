...
func TestOpenAIGatewayService_SelectAccountWithScheduler_OverLimitModeUsesStrictPriorityBeforeLoadBalance(t *testing.T) {
	ctx := context.Background()
	groupID := int64(51103)
	rateLimitedUntil := time.Now().Add(30 * time.Minute)
	primary := Account{
	...
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	cfg := &config.Config{}
	// Keep the production default behavior: TopK > 1 means normal scheduler
	// weighting can otherwise choose a lower-priority healthy account first.
	svc := &OpenAIGatewayService{
		accountRepo:        productionLikeOpenAIOverLimitRepoStub{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{primary, backupA, backupB}}},
		cfg:                cfg,
		rateLimitService:   &RateLimitService{settingService: settingService},
		concurrencyService: NewConcurrencyService(schedulerTestConcurrencyCache{}),
	}
	...
	selection, decision, err := svc.SelectAccountWithScheduler(
		ctx,
		&groupID,
		"resp_overlimit_priority_first",
		"session_overlimit_priority_first",
		"gpt-5.1",
		nil,
		OpenAIUpstreamTransportAny,
		false,
...
func TestOpenAIGatewayService_SelectAccountWithScheduler_OverLimitModeFallsBackToDBWhenRateLimitedAccountMissingFromSnapshot(t *testing.T) {
	...
	resetOpenAIAdvancedSchedulerSettingCacheForTest()
	repo := productionLikeOpenAIOverLimitRepoStub{stubOpenAIAccountRepo: stubOpenAIAccountRepo{accounts: []Account{primary, backup}}}
	svc := &OpenAIGatewayService{
		accountRepo:        repo,
		cfg:                &config.Config{},
		rateLimitService:   &RateLimitService{settingService: settingService},
		...
		schedulerSnapshot: NewSchedulerSnapshotService(
			&openAISnapshotCacheStub{
				snapshotAccounts: []*Account{&backup},
				accountsByID: map[int64]*Account{
					backup.ID: &backup,
				},
			},
